...
def find_new_pred(pred_all, times_p, error_list_p_1, error_list_p_2, error_list_p_3):   # 看看平票的情况多不
    ...
    if max_time != 1:
        ...
        error_list_p_min = []
        error_list_p_min.append(np.mean(error_list_p_1))
        error_list_p_min.append(np.mean(error_list_p_2))
        error_list_p_min.append(np.mean(error_list_p_3))
        idx_min = np.argmin(error_list_p_min)
        return pred_all[idx_min]
...
if __name__ == "__main__":
    ...
    for img_name in os.listdir(data_dir):
        ...
        if flag: # 小于msp 需要再分类 这里可能有对的分错 也可能有错的分对
            ...
            error_1_image_pos_1 = eval_error_pos(unet, scheduler, x0, noise_all_pos = noise_all_pos_1, ts=ts, noise_idxs=noise_idxs, text_emb_idxs = text_emb_idxs)
            error_1_image_pos_2 = eval_error_pos(unet, scheduler, x0, noise_all_pos = noise_all_pos_2, ts=ts, noise_idxs=noise_idxs, text_emb_idxs = text_emb_idxs) #256分辨率的
