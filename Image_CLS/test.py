...
# model1 = timm.create_model('resnet18.tv_in1k', pretrained=True, num_classes=100)
# model1.eval()
...
if __name__ == "__main__":
    ...
    # model = build_model("resnet18", cfg.num_cls, pretrained=True)
    model = models.resnet50()  # 1000  fc
    in_features = model.fc.in_features
    fc = nn.Linear(in_features=in_features, out_features=100)
    model.fc = fc
    # 载入训练好的权重
    model_weights = r"D:\LCX_IMG_CLS\ouputs\20240212-1526\best.pth"
    ...
    val_dataloader = torch.utils.data.DataLoader(testset, batch_size=256, shuffle=False)
    ...
    loss_valid, acc_valid, conf_mat_valid, path_error_valid, acc_top5_valid, total_msp = ModelTrainer.valid_one_epoch(
            val_dataloader, 
            model, loss_fn, 
            device=device,
        )
    ...
    f=open("msp_resnet18cifar100.txt","w")
    f.writelines(str(total_msp))
    f.close()

    length = len(total_msp)
    # print(length)
    p = 0.05 * length
    p = math.ceil(p)
    # print(p)
    print("0.05, 0.03, 0.01msp:")
    total_msp.sort(reverse=False)
    print(total_msp[p])
