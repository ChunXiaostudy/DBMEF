...
def get_transform(interpolation=InterpolationMode.BICUBIC, size=512): # 转化到[3, 512, 512]
...
def center_crop_resize(img, interpolation=InterpolationMode.BILINEAR):  # 将[3, 512, 512]做中心化, normalize
    transform = get_transform(interpolation=interpolation)
    return transform(img)

#  sampling N piars (ti, ebsioni)
def eval_prob_adaptive(unet, latent, text_embeds, scheduler, args, latent_size=64, all_noise=None):
    ...
    if all_noise is None:
        ...
        # noise shape [max_n_smaples*n_trials, 4, latentsize,latensize] 传入nosie的方式
        # 加入的噪声形状要和latent变量大小一样
...
def main():
    ...
    # args for adaptively choosing which classes to continue trying     # sample strategy
    ...
    parser.add_argument('--n_samples', nargs='+', type=int, default=[50, 100])
    ...
    # target_dataset = get_target_dataset(args.dataset, train=args.split == 'test', transform=transform)
